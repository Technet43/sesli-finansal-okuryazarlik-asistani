...
# Regex'ler tavsiye dilini temizler. AI çıktısı her zaman buradan geçer.
FORBIDDEN_PATTERNS = [
...
# Sade Türkçe finans terimleri sözlüğü — UI'da tooltip olarak gösterilir.
GLOSSARY = {
...
# Şüpheli paternleri yakalayan kurallar (manipülasyon dedektörü).
ANOMALY_RULES = {
...
def detect_anomalies(disclosures: list[dict]) -> list[tuple[str, str]]:
    """KAP bildirimlerinde şüpheli paternleri yakalar."""
    if not disclosures:
        return []
    ...
    if any(d.get("is_late") for d in disclosures):
        ...
        flags.append(("⏰ Geç bildirim", f"{late_count} bildirim geç gönderilmiş. " + ANOMALY_RULES["geç_bildirim"]))

    correctives = sum(1 for d in disclosures if d.get("is_corrective"))
    if correctives >= 2:
        flags.append(("✏️ Çoklu düzeltme", f"{correctives} düzeltme bildirimi. " + ANOMALY_RULES["düzeltme"]))
    ...
    if important_no_attach:
        flags.append(("📎 Eksik ek dosya", f"{len(important_no_attach)} önemli bildirimde ek dosya yok. " + ANOMALY_RULES["ek_eksik"]))

    # Gece geç saatte gönderilenler (23:00 - 06:00)
    late_night = 0
    for d in disclosures:
        ts = d.get("publish_datetime", "")
        ...
        if match:
            ...
            if hour >= 23 or hour < 6:
                late_night += 1
    if late_night >= 2:
        flags.append(("🌙 Geç saat bildirim", f"{late_night} bildirim gece 23:00-06:00 arasında gönderilmiş; bu olağandışıdır."))

    return flags


def apply_glossary(text: str) -> str:
    """Metindeki finans terimlerini HTML <abbr title="..."> tooltip'leri ile sarar."""
    if not text:
        return ""
    out = escape(text)
    # Uzun terimleri önce eşleştir (kira sertifikası > kira)
    for term in sorted(GLOSSARY.keys(), key=len, reverse=True):
        explanation = GLOSSARY[term]
        # Kelime sınırı kontrolü, case-insensitive
        pattern = re.compile(rf"(?<![a-zçğıöşüA-ZÇĞİÖŞÜ])({re.escape(term)})(?![a-zçğıöşüA-ZÇĞİÖŞÜ])", re.IGNORECASE)
        out = pattern.sub(
            lambda m: f'<abbr class="gloss" title="{escape(explanation)}">{m.group(1)}</abbr>',
            out,
        )
    return out
